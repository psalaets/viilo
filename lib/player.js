...
playerSchema.statics.rank = function() {
  return this.find().exec().then(function(players) {
    ...
    var savePlayerPromises = rankings
      .map(assignNewRankings)
      .map(assignNewTiers)
      .map(savePlayer);

    return Promise.all(savePlayerPromises);

    function assignNewRankings(ranking) {
    ...
    function assignNewTiers(player) {
      var newTier = tier.byRank(player.rank);
      player.tier = newTier.id;
